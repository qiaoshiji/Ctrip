...
set.seed(0)
xgb_fit <- xgboost(data = data.matrix(train[,-4]),
...
pre_xgboost <- predict(xgb_fit ,data.matrix(test))
sum(pre_xgboost<0)
sum(pre_xgboost>product_info$maxstock*30)
names <- colnames(train[,-4])
...
my_prediction
fwrite(my_prediction,"./model_result/model4_result.csv")
